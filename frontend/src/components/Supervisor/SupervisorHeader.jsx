...
export function SupervisorHeader({ onToggleSidebar, cartCount = 1, unreadCount = 0, onNavigate }) {
  return (
    <header className="flex items-center gap-4 border-b border-slate-200 bg-white px-6 py-3">
      ...
      <button type="button" onClick={() => onNavigate('notifications')} className="relative text-slate-600">
        <Bell size={20} />
        {unreadCount > 0 ? (
          <span className="absolute -right-2 -top-2 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] text-white">
            {unreadCount}
